...
2. Each todo file is built from `LXC-BRM-context/requirements.md` (the in-repo copy of the functional requirements) plus the recorded decisions in `LXC-BRM-context/decisions/` — not invented ad hoc.
...
7. If a requirement conflicts with a recorded decision, the decision wins, and the conflict gets written down in `LXC-BRM-context/decisions/`.
...
- Any other BRM folder that needs a task tracked: write the task here, name the folder in the text, link back to this file from that folder's README.
